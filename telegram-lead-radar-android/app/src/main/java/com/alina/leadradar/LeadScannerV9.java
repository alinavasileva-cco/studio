...
/**
 * v9 scanner.
 * Each Telegram post can contain many independent orders. The post is first
 * expanded so hidden t.me links become visible @usernames, then split into
 * separate task blocks. Only blocks with their own direct Telegram contact are kept.
 */
public final class LeadScannerV9 {
    ...
    private static final Pattern ACTION_BULLET_START = Pattern.compile("(?ium)(?=^\\s*[•▪◾●✅☑🔹🔸⭐👉✍📌]\\s*(?:#|ищ|нуж|треб|задач|сдел|созд|оформ|разраб))");
    ...
    private static final String[] PRESENTATION_BUILD = {
            ...
            "сделать слайды", "дизайн слайдов", "оформление слайдов"
    };
    ...
    private static final String[] PRESENTATION_DIRECT_REQUEST = {
            ...
            "ищу на презентац", "нужен человек на презентац"
    };
    ...
    private String textWithVisibleTelegramContacts(Element textNode, Element message, String channel) {
        ...
        if (!HANDLE.matcher(text).find()) {
            Set<String> messageUsers = new HashSet<>();
            for (Element a : message.select("a[href]")) {
                String user = usernameFromTelegramHref(a.attr("href"));
                if (isAllowedUsername(user, channel)) messageUsers.add(user);
            }
            if (messageUsers.size() == 1) {
                text = text + "\nКонтакт: @" + messageUsers.iterator().next();
            }
        }
        return text;
    }
    ...
    private List<String> splitTasks(String text, String channel) {
        List<String> structural = splitByStructuralStarts(text);
        List<String> out = new ArrayList<>();
        for (String block : structural) {
            out.addAll(splitByResponseContacts(block, channel));
        }
        if (out.isEmpty()) out.add(text);
        return out;
    }

    private List<String> splitByStructuralStarts(String text) {
        TreeSet<Integer> starts = new TreeSet<>();
        starts.add(0);
        collectStarts(text, NUMBER_START, starts);
        collectStarts(text, HASH_START, starts);
        collectStarts(text, ACTION_BULLET_START, starts);
    ...
    private List<String> splitByResponseContacts(String text, String channel) {
        ...
        while (m.find()) {
            ...
            int to = Math.min(text.length(), m.end() + 80);
            String around = text.substring(from, to).toLowerCase(Locale.ROOT);
            boolean likelyResponse = scoreContactContext(around) >= 2
                    || isNearLineEnd(text, m.end())
                    || m.end() >= text.length() - 100;
            if (likelyResponse) ends.add(m.end());
        }
    ...
    private boolean isNearLineEnd(String text, int pos) {
        int nl = text.indexOf('\n', pos);
    ...
    private String chooseDirectTelegramContact(String taskText, String channel) {
        String best = null;
        ...
        while (m.find()) {
            ...
            if (isNearLineEnd(taskText, m.end())) score += 3;
            if (m.end() >= taskText.length() - 120) score += 3;
