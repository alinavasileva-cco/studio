...
public final class MainActivityV7 extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Universal Lead Radar v7");
    ...
    private View buildUi() {
        ...
        title.setText("Universal Telegram Lead Radar v7");
        ...
        subtitle.setText("РУЧНОЙ ПОИСК БЫСТРЫХ ЗАКАЗОВ.\n"
                ...
                + "Обычные вакансии, Product/Project Manager, продажники, монтаж, Tilda и WordPress отбрасываются. "
                + "Добавлены быстрые потоки Kwork/YouDo и возвращены удачные старые Telegram-источники.");
        ...
        periodHelp.setText("По умолчанию 7 дней. Каждый источник листается назад до выбранной даты.");
        periodHelp.setTextSize(13);
        root.addView(periodHelp);

        root.addView(label("Источники быстрых заказов"));
        TextView channelHelp = new TextView(this);
        channelHelp.setText("Сначала идут каналы с разовыми заказами и прямыми контактами, затем дизайн-фриланс и агрегаторы быстрых заданий. "
                + "Широкие карьерные каналы, давшие ложные Product Manager / Project Manager / sales результаты, из ядра удалены. Список можно редактировать.");
        ...
        openContact.setText("ОТКРЫТЬ КОНТАКТ / БЫСТРОЕ ЗАДАНИЕ");
        openContact.setOnClickListener(v -> {
            String ref = store.lastPreviewUser();
            String url = LeadStore.contactUrl(ref);
            openUrl(url, "Пока нет найденного контакта или ссылки на задание");
        });
        ...
        copyMessage.setOnClickListener(v -> {
            ...
            if (message == null || message.isEmpty()) {
                Toast.makeText(this, "Пока нет подготовленного отклика", Toast.LENGTH_SHORT).show(); return;
            }
        ...
        clear.setOnClickListener(v -> {
            if (store.running()) {
                Toast.makeText(this, "Сначала останови текущий поиск", Toast.LENGTH_SHORT).show(); return;
            }
            store.clearPreviewHistory(); updateStatus(); updateResults();
        });
        root.addView(clear, buttonParams());

        root.addView(label("Найденные быстрые заказы"));
        TextView hint = new TextView(this);
        hint.setText("Для прямых Telegram-заявок показывается Telegram-контакт. Для потоков Kwork/YouDo — ссылка на само задание. Ничего автоматически не отправляется.");
        hint.setTextSize(13);
        root.addView(hint);
    ...
    private void startManualSearch() {
        ...
        Toast.makeText(this, "Ручной поиск быстрых заказов запущен", Toast.LENGTH_LONG).show();
    }

    private void stopManualSearch() {
        if (!store.running()) { Toast.makeText(this, "Сейчас поиск не запущен", Toast.LENGTH_SHORT).show(); return; }
        store.requestStop();
        Intent i = new Intent(this, ScannerService.class); i.setAction(ScannerService.ACTION_STOP); startService(i);
        updateStatus();
    }
    ...
    private void updateResults() {
        ...
        results.setText(history == null || history.isEmpty()
                ? "Пока быстрых заказов нет. Выбери период и запусти поиск."
                : history);
    ...
    private TextView label(String text) {
        TextView v = new TextView(this); v.setText(text); v.setTextSize(15); v.setTypeface(Typeface.DEFAULT_BOLD);
        v.setPadding(0, dp(12), 0, dp(4)); return v;
    }
