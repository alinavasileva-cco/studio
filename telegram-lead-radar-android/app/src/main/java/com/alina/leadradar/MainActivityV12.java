...
public final class MainActivityV12 extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Universal Lead Radar v12");
    ...
    private View buildUi() {
        ...
        title.setText("Universal Telegram Lead Radar v12");
        title.setTextSize(24);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        root.addView(title);

        TextView subtitle = new TextView(this);
        subtitle.setText("РУЧНОЙ ПОИСК · НИКАКОЙ АВТООТПРАВКИ.\n"
                + "v12 исправляет дайджесты: пункты вида «6. #Дизайнер_презентаций … 📝 @контакт» связываются с собственным Telegram-контактом. "
                + "Каждая найденная заявка — отдельная карточка с галочкой. Для презентаций используется отдельное портфолио презентаций.");
        ...
        TextView periodHint = new TextView(this);
        periodHint.setText("Для твоего сценария рекомендую 3 дня: каждый новый запуск полностью пересобирает выдачу.");
        periodHint.setTextSize(13);
        root.addView(periodHint);
        ...
        hint.setText("Галочка только отмечает, кому ты решила написать. Ничего автоматически не отправляется.");
        hint.setTextSize(13);
        root.addView(hint);
    ...
    private void startManualSearch() {
        ...
        Toast.makeText(this, "Свежий поиск v12 запущен", Toast.LENGTH_LONG).show();
        uiHandler.postDelayed(() -> { updateStatus(); renderResults(true); }, 700L);
    }
    ...
    private void renderResults(List<LeadRecord> records) {
        ...
        if (records.isEmpty()) {
            ...
            empty.setText(store.running() ? "Идёт поиск…" : "Пока подходящих заявок нет.");
    ...
    private View buildLeadCard(LeadRecord r) {
        ...
        choose.setOnCheckedChangeListener((buttonView, checked) -> {
    ...
    private void openFirstSelected() {
        ...
        Toast.makeText(this, "Сначала отметь хотя бы одну заявку", Toast.LENGTH_LONG).show();
    }

    private void updateSelectedSummary() {
        if (selectedSummary != null) selectedSummary.setText("Отмечено к ручной отправке: " + store.selectedCount());
    }
    ...
    private int countChannels(String raw) {
        ...
        for (String piece : raw.split("[\\n,; ]+")) {
            String c = LeadScannerV12.normalizeChannel(piece);
