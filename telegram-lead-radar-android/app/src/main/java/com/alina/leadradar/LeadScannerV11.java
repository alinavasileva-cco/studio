...
/** v11: public Telegram reader with robust direct-contact extraction. */
public final class LeadScannerV11 {
    ...
    private Set<String> collectUsersFromMessage(Element message, String channel) {
        ...
        Matcher raw = TME_RAW.matcher(message.html());
        while (raw.find()) {
    ...
    private String exposeRawTelegramUrls(String text, String channel) {
        ...
        while (m.find()) {
            ...
            if (allowed(u, channel)) repl = repl + " @" + u;
    ...
    private String chooseContact(String text, String channel) {
        ...
        while (m.find()) {
            String u = m.group(1);
            if (!allowed(u, channel)) continue;
            int from = Math.max(0, m.start() - 150), to = Math.min(text.length(), m.end() + 100);
            String around = text.substring(from, to).toLowerCase(Locale.ROOT);
            int score = scoreContext(around) + (nearLineEnd(text, m.end()) ? 3 : 0) + (m.end() >= text.length() - 150 ? 3 : 0);
    ...
    private int scoreContext(String s) {
        ...
        if (s.contains("✍") || s.contains("📩") || s.contains("👉") || s.contains("📨")) n += 4;
        return n;
    }
