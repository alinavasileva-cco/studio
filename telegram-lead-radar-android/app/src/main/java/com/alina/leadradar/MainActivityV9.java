...
public final class MainActivityV9 extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Universal Lead Radar v9");
    ...
    private View buildUi() {
        ...
        title.setText("Universal Telegram Lead Radar v9");
        ...
        subtitle.setText("РУЧНОЙ ПОИСК · СВЕЖИЙ СНИМОК КАЖДЫЙ ЗАПУСК.\n"
                + "При новом запуске старая выдача очищается и выбранные 24 часа / 3 / 7 / 14 дней проверяются заново. "
                + "Большие Telegram-посты разбираются на отдельные задания. В результат попадают только презентации и простые сайты/лендинги с прямым Telegram-контактом.");
        ...
        periodHelp.setText("Каждый ручной запуск заново листает публичную историю каждого канала до выбранной даты. Старые результаты предыдущего запуска не блокируют новые.");
        ...
        channelHelp.setText("Если в одном посте опубликовано несколько заказов, v9 старается разделить их по нумерации, хэштегам, маркерам и отдельным Telegram-контактам. "
                + "Kwork/YouDo и внешние биржи по-прежнему не принимаются как контакт.");
        ...
        hint.setText("Это не накопительная история: здесь показывается свежий результат последнего ручного прохода за выбранный период.");
        hint.setTextSize(13);
        root.addView(hint);
    ...
    private void startManualSearch() {
        ...
        Toast.makeText(this, "Свежий поиск запущен. Предыдущая выдача будет заменена результатами этого прохода.", Toast.LENGTH_LONG).show();
        uiHandler.postDelayed(() -> { updateStatus(); updateResults(); }, 700L);
    }
    ...
    private void updateResults() {
        ...
        results.setText(history == null || history.isEmpty()
                ? (store.running() ? "Идёт свежий проход. Найденные заявки будут появляться здесь по мере проверки каналов."
                : "Пока подходящих заявок нет. Выбери период и запусти свежий ручной поиск.")
                : history);
