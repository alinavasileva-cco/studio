...
/** v10: robust public Telegram reader + per-post multi-task parsing. */
public final class LeadScannerV10 {
    ...
    private static final String[] RESPONSE_WORDS = {
            ...
            "личку","лс","присылайте","портфолио","обращаться","обращайтесь"
    };
    ...
    private PageState processDocument(Document doc, String channel, Instant cutoff, LeadStore store,
        ...
        for (Element message : messages) {
            ...
            for (String block : blocks) {
                ...
                String key = category.name() + "|" + user.toLowerCase(Locale.ROOT) + "|" + Integer.toHexString(normalize(text).hashCode());
    ...
    private String textWithContacts(Element textNode, Element message, String channel) {
        ...
        for (Element a : copy.select("a[href]")) {
            ...
            if (!a.text().toLowerCase(Locale.ROOT).contains(u.toLowerCase(Locale.ROOT))) a.after(" @" + u);
        }
    ...
    private List<String> splitTasks(String text, String channel) {
        TreeSet<Integer> starts = new TreeSet<>(); starts.add(0);
        collect(text, NUMBER_START, starts); collect(text, HASH_START, starts); collect(text, BULLET_START, starts);
    ...
    private List<String> splitByContacts(String text, String channel) {
        Matcher m = HANDLE.matcher(text); List<Integer> ends = new ArrayList<>();
        while (m.find()) {
            String u = m.group(1); if (!allowed(u, channel)) continue;
            int from = Math.max(0, m.start()-120), to = Math.min(text.length(), m.end()+80);
            String around = text.substring(from,to).toLowerCase(Locale.ROOT);
            if (scoreContext(around) >= 2 || nearLineEnd(text,m.end()) || m.end() >= text.length()-100) ends.add(m.end());
        }
        if (ends.size() < 2) return java.util.Collections.singletonList(text);
        List<String> out = new ArrayList<>(); int start=0;
        for (int end : ends) { String p=text.substring(start,end).trim(); if (p.length()>=10) out.add(p); start=end; }
    ...
    private String chooseContact(String text,String channel) {
        Matcher m=HANDLE.matcher(text); String best=null; int bestScore=Integer.MIN_VALUE;
        while(m.find()) {
            String u=m.group(1); if(!allowed(u,channel)) continue;
            int from=Math.max(0,m.start()-120), to=Math.min(text.length(),m.end()+80);
            String around=text.substring(from,to).toLowerCase(Locale.ROOT);
            int score=scoreContext(around)+(nearLineEnd(text,m.end())?3:0)+(m.end()>=text.length()-120?3:0);
            if(around.contains("канал")||around.contains("подпис")) score-=5;
            if(score>bestScore){bestScore=score;best=u;}
        }
        return bestScore>=2?best:null;
    }
    private int scoreContext(String s){int n=0; for(String w:RESPONSE_WORDS) if(s.contains(w)) n+=3; if(s.contains("✍")||s.contains("📩")||s.contains("👉")) n+=4; return n;}
    private boolean nearLineEnd(String text,int pos){int nl=text.indexOf('\n',pos); if(nl<0) nl=text.length(); return nl-pos<=18;}
    private boolean allowed(String u,String channel){if(u==null||!USERNAME.matcher(u).matches()||u.equalsIgnoreCase(channel)) return false; String l=u.toLowerCase(Locale.ROOT); return !l.endsWith("bot")&&!l.contains("kwork")&&!l.contains("youdo");}

    private String usernameFromHref(String href){
        if(href==null)return null; String h=href.trim(), l=h.toLowerCase(Locale.ROOT);
        int p=l.indexOf("t.me/"); int len=5; if(p<0){p=l.indexOf("telegram.me/");len=12;} if(p<0)return null;
        String path=h.substring(p+len); int q=path.indexOf('?'); if(q>=0)path=path.substring(0,q); int hash=path.indexOf('#');if(hash>=0)path=path.substring(0,hash);
        while(path.startsWith("/"))path=path.substring(1); if(path.startsWith("s/")||path.startsWith("c/")||path.startsWith("+")||path.contains("/"))return null;
        while(path.startsWith("@"))path=path.substring(1); return USERNAME.matcher(path).matches()?path:null;
    }
    private String extractBudget(String text){Matcher m=BUDGET.matcher(text); return m.find()?(m.group(1)+" "+m.group(2)).replace('\u00A0',' ').trim():"";}
    private Instant parsePostedAt(Element message){Element t=message.selectFirst("time[datetime]"); if(t==null)return null; String raw=t.attr("datetime"); try{return OffsetDateTime.parse(raw).toInstant();}catch(Exception e){try{return Instant.parse(raw);}catch(Exception x){return null;}}}
    private long parsePostId(String data){int s=data.lastIndexOf('/'); if(s<0)return -1; try{return Long.parseLong(data.substring(s+1));}catch(Exception e){return -1;}}
    private String normalize(String text){return text.toLowerCase(Locale.ROOT).replaceAll("https?://\\S+"," ").replaceAll("@[A-Za-z0-9_]{5,32}"," ").replaceAll("\\s+"," ").trim();}
    private void collect(String text,Pattern p,Set<Integer>s){Matcher m=p.matcher(text);while(m.find())s.add(m.start());}
    private List<String> slice(String text,TreeSet<Integer>starts){List<Integer> a=new ArrayList<>(starts);List<String>o=new ArrayList<>();for(int i=0;i<a.size();i++){int f=a.get(i),t=i+1<a.size()?a.get(i+1):text.length();if(t>f){String p=text.substring(f,t).trim();if(p.length()>=10)o.add(p);}}return o.isEmpty()?java.util.Collections.singletonList(text):o;}
    private boolean containsAny(String s,String...n){for(String x:n)if(x!=null&&!x.isEmpty()&&s.contains(x))return true;return false;}
    private int countCyrillic(String s){int n=0;for(int i=0;i<s.length();i++){char c=s.charAt(i);if((c>='А'&&c<='я')||c=='ё'||c=='Ё')n++;}return n;}

    public static String normalizeChannel(String raw){if(raw==null)return"";String s=raw.trim();s=s.replace("https://t.me/s/","").replace("https://t.me/","").replace("http://t.me/","");while(s.startsWith("@"))s=s.substring(1);int slash=s.indexOf('/');if(slash>=0)s=s.substring(0,slash);int q=s.indexOf('?');if(q>=0)s=s.substring(0,q);return USERNAME.matcher(s).matches()?s:"";}
    private static final class PageState { int messages,dated,fresh; long minPostId=Long.MAX_VALUE; }
}
